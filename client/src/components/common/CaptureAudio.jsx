...
export default ({hide}) => {
  ...
  const handleStopRecording = () => {
    if(mediaRecorderRef.current && isRecording) {
      ...
      const audioChunks = [];
      mediaRecorderRef.current.addEventListener('dataavaible', (event) => {
    ...
    try {
      ...
      if(response.status === 201) {
        socket.current.emit('send-msg',{
          message: response.data.message, from: userInfo?.id, to: currentChatUser?.id
        });
        dispatch({
  ...
  return (
    <div className="flex text-2xl w-full justify-end items-center">
      ...
      <div className="mx-4 py-2 px-4 text-white text-lg flex gap-3 justify-center items-center bg-search-input-container-background rounded-full drop-shadow-xl">
        {
          isRecording ?
          <div className="text-red-500 animate-pulse 2-60 text-center">Recording <span>{recordingDuration}s</span></div> :
          <div className="holacesar">
            {
              recordedAudio && 
              <>
                {
