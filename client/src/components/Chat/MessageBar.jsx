  ...
  const sendMessage = async () => {
    try {
      const { data } = await axios.post(ADD_MESSAGE_ROUTE,{
        message, from: userInfo?.id, to: currentChatUser?.id
      });
      socket.current.emit('send-msg',{
        message: data?.message, from: userInfo?.id, to: currentChatUser?.id
      });
      dispatch({
        type: reducerCases.ADD_MESSAGE,
        newMessage: {
            ...
            <button>
              {
                ...
                <FaMicrophone className="text-panel-header-icon cursor-pointer text-xl" title="Record message" onClick={() => setShowAudioRecorder(true)} />
              }
            </button>
          </div>
        </>
